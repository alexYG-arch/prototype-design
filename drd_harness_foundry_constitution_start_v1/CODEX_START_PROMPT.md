...
9. `drd-harness compile-source-preserving-drd` 只是 source-preserving 保守编译，不是完整 staged execution；payload 必须声明 `staged_execution_complete=false`。`generate-drd` 只能作为兼容 alias。
...
13. `SOURCE_PRESERVING_DRD.md` 必须是保源规格正文；完整 staged execution 的 `DRD-05/FINAL_DRD.md` 仍保留最终规格命名。正文不得拼接候选文档、不得包含 hash/source/review/gate/run_state 过程证据、不得 dump 完整 inventory；这些证据只能进入 manifest/reference/hash/QA sidecar。
