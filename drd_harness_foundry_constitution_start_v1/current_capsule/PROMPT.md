...
10. 将用户指定 PRD 做 source-preserving 保守 DRD 编译时才使用 `drd-harness generate-drd`；该命令不是完整 staged execution，payload 必须声明 `staged_execution_complete=false`。
...
14. `DRD-05/FINAL_DRD.md` 必须是 reader-facing final spec；hash、source path、review decision、gate 状态、run_state、候选标签和完整 inventory dump 必须保存在 sidecar，不得进入正文。
